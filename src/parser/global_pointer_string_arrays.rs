...
use super::global_string_pointer_arrays::parse_string_pointer_array_initializer;
...
pub(super) fn parse_global_pointer_string_array(
    tokens: &[Token],
    constants: &[Constant],
    ...
    let Ok(values) =
        parse_string_pointer_array_initializer(&declaration[assign_index + 1..], constants)
    else {
        return Ok(None);
    };
...
fn pointer_array_length(
    ...
    if initializer_count > length {
        return Err(CompileError::new(
            "too many global pointer-array string initializers",
        ));
    }
    Ok(length)
}
