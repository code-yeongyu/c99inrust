...
#[test]
fn global_double_array_designators_zero_fill_matches_host_stdout_and_exit_code() {
    // given
    let source = "int puts(char*); double values[4] = { [2] = 5.0, [0] = 1.0 }; int main(void) { puts(\"global-double-array-designators\"); return values[0] == 1.0 && values[1] == 0.0 && values[2] == 5.0 && values[3] == 0.0 ? 0 : 1; }\n";

    // when/then
    assert_compile_run_matches_host(OracleCase {
        name: "global_double_array_designators_zero_fill",
        source,
    });
}
...
#[test]
fn global_complex_double_array_designators_infer_length_matches_host_stdout_and_exit_code() {
    // given
    let source = "int puts(char*); double _Complex values[] = { [2] = 3.0, 4.0, [0] = 1.0, 2.0 }; int main(void) { double *raw = (double *)values; puts(\"global-complex-array-designators\"); return sizeof(values) == 4 * sizeof(double _Complex) && raw[0] == 1.0 && raw[1] == 0.0 && raw[2] == 2.0 && raw[3] == 0.0 && raw[4] == 3.0 && raw[5] == 0.0 && raw[6] == 4.0 && raw[7] == 0.0 ? 0 : 1; }\n";

    // when/then
    assert_compile_run_matches_host(OracleCase {
        name: "global_complex_double_array_designators_infer_length",
        source,
    });
}
