...
use super::aarch64_loads::{
    emit_aarch64_load_global, emit_aarch64_load_global_bool,
    emit_aarch64_load_global_byte_subscript, emit_aarch64_load_global_f32_as_f64,
    ...
    match expr {
        LoweredExpr::Global { name, scalar_type } => {
            if *scalar_type == ScalarType::Bool {
                return emit_aarch64_load_global_bool(name, labels.target, assembly);
            }
