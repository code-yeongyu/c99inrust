...
pub(in crate::codegen) fn emit_aarch64_complex_expression_argument(
    arg: &LoweredExpr,
    ...
    {
        let mut materializer = Aarch64ComplexArgMaterializer {
            scalar_type,
            temporary_base,
            depth,
            labels,
    ...
    load_complex_expression_registers(scalar_type, temp_offset, first_register, assembly)
}

struct Aarch64ComplexArgMaterializer<'a, 'label> {
    scalar_type: ScalarType,
    temporary_base: usize,
    depth: usize,
...
impl Aarch64ComplexArgMaterializer<'_, '_> {
    ...
    fn emit_conditional_value_to_temp(
        ...
        emit_aarch64_compare_result_to_zero(expr_width(condition), self.assembly)?;
        write_assembly!(self.assembly, "\tb.eq {else_label}\n")?;
        self.emit_value_to_temp(then_expr, temp_offset)?;
        write_assembly!(self.assembly, "\tb {end_label}\n")?;
    ...
    fn store_lane(&mut self, index: usize, temp_offset: usize) -> CompileResult<()> {
        match self.scalar_type {
            ScalarType::ComplexFloat => {
                self.assembly.push_str("\tfcvt s0, d0\n");
                write_assembly!(
                    self.assembly,
                    "\tstr s0, [sp, #{}]\n",
                    temp_offset + (index * 4)
            ...
            ScalarType::ComplexDouble => write_assembly!(
                self.assembly,
                "\tstr d0, [sp, #{}]\n",
                temp_offset + (index * 8)
            ),
