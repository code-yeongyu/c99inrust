...
pub fn assert_compile_run_matches_host(case: OracleCase) {
    if cfg!(windows) || !command_exists("cc") {
        return;
    }
    assert_case_matches_host(case, C99InRustMode::Compile);
}

pub fn assert_macos_compile_run_matches_host(case: OracleCase) {
...
pub fn assert_build_run_matches_host(case: OracleCase) {
    if cfg!(windows) || !command_exists("cc") {
        return;
    }
    assert_case_matches_host(case, C99InRustMode::Build);
}
...
fn assert_case_matches_host(case: OracleCase, mode: C99InRustMode) {
    // given
